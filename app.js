...
function companyView(){
 ...
 const paint=()=>{
   ...
   $('#companies').innerHTML=arr.map(t=>{
      ...
      return `<article class="card"><div class="company-row"><div><div class="title">${esc(t.name)}</div><div class="company">${esc(t.region)}</div><div class="badges"><span class="badge fit">Fit ${t.fit}/10</span><span class="badge">${current} monitored jobs</span>${fresh?`<span class="badge new">${fresh} new</span>`:''}</div></div><div class="right"><div class="application-count">${appsFor(t)}</div><div class="small">applications</div></div></div><div class="source-row"><a class="btn primary" href="${esc(t.careers_url)}" target="_blank" rel="noopener">Official Careers</a><a class="btn linkedin" href="${esc(t.linkedin_url)}" target="_blank" rel="noopener">LinkedIn Jobs</a><a class="btn indeed" href="${esc(t.indeed_url)}" target="_blank" rel="noopener">Indeed Jobs</a></div><div class="actions"><button class="btn" data-plus="${t.id}">+ Application</button><button class="btn" data-minus="${t.id}">−1</button></div></article>`;
   }).join('');
